  ...
  try {
    if (modelId === "text-davinci-003") {
      const completion = await openai.createCompletion({
        model: modelId, prompt: generatePrompt(animal), temperature: 0.6,
      });
      res.status(200).json({result: completion.data.choices[0].text});
    } else if (modelId === "gpt-3.5-turbo") {
      console.log(generateMessage(animal))
      const chatCompletion = await openai.createChatCompletion({
        model: modelId, messages: generateMessage(animal), temperature: 0.6,
      });
      res.status(200).json({
...
function generateMessage(animal) {
  ...
  return [{
    ...
    role: "user", content: "Animal: " + capitalizedAnimal
  }]
}
